/**
 * jQuery Real Ajax Uploader 2.6.4
 * http://www.albanx.com/
 *
 * Copyright 2012, Alban Xhaferllari
 *
 * Date: 29-01-2013
 */
...
{	
	...
	var fileObject = function(file, name, size, ext, AU)
	{
		...
		this.size		= size; //size of file, not visible at once in IE
    	...
    	if(AU.hasHtml4)
    	{
        	...
        	form.append('<input type="hidden" value="'+encodeURIComponent(name)+'" name="ax-file-name" />');//input for re-name of file
        	this.xhr = form;
    	}
	...
	fileObject.prototype.uploadAjax = function(){
		...
		//var isfirefox5 = ( $.browser.mozilla && parseInt($.browser.version) < 6 );
		var isfirefox5 =  (navigator.userAgent).match(/Firefox\/(\d+)?/);
		var fire_ver = isfirefox5!==null && isfirefox5[1]!==undefined && !isNaN(isfirefox5[1]) ? parseInt(isfirefox5[1]) : 7;
		
		if(window.FormData !== undefined && fire_ver>6)//firefox 5 formdata does not work correctly
		{
	...
	AjaxUploader.prototype.addFiles = function(files)
	{
		...
		{
			var ext, name, size, pos = this.files.length;
		...
		//add sort of files if jquery-ui is included and if option is enabled
	...
	AjaxUploader.prototype.checkFile = function(name, size)
	{
		...
		if(!max_num_f)	console.log('Max files number reached:' + max_num_f);
		if(!allow_ext)	console.log('Extension not allowed:' + ext);
		if(!max_size)	console.log('File size now allowed:' + size);
	...
	/**
	 * Function that calculates the URL for sending more parameters such as user parameters
	 * and thumbnail generation information
	 * @param file_name
	 * @param size
	 * @returns upload url
	 */
	AjaxUploader.prototype.getUrl = function(file_name, size)//TODO post this data in POST
	{
		var settings = this.settings;
    ...
    var globalSettings = 
    {
    	...
    	infoBox:		false,						// tell if to show info upload speed, or where to show the infos
	...
	var methods =
	{
		...
		getUrlFlash: function(name, size)
		{
			...
			//strange but IE needs the path not relative to the flash file
