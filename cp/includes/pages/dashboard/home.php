...
.dashx-inner-details-under {
	font-size: 32px;
	line-height: 1em;
	padding-bottom: 15px;
}
.dashx-inner-details-under-container {
	display: table;
	margin: 0 auto;
	position: relative;
	left: -20px;
}
.dashx-inner-details-under-container-row {
	display: table-row;
}
.dashx-inner-details-under-container-row-col1 {
	display: table-cell;
	width: 50%;	
}
.dashx-inner-details-under-container-row-col2 {
	display: table-cell;
}
...
if(mysql_num_rows($CHECK_EAG)<1){
	...
	while($FETCH_CHECK_EAG=mysql_fetch_array($CHECK_EAG)){
		if(getGlobalVars($properties,'display_social_modal')=="no"){/* NO SOCIAL MODAL */$COUNT_CHECK_EAG_EXITS="<a class=\"red question\" title=\"No Social Modal Detected to Register Exits\">NIL</a>";$COUNT_CHECK_EAG_GOTHRUS+=1;}else{if($FETCH_CHECK_EAG['fb_like']=="yes"){$COUNT_CHECK_EAG_GOTHRUS+=1;}else if($FETCH_CHECK_EAG['fb_like']=="no"){$COUNT_CHECK_EAG_EXITS+=1;}}
	}
}

$dashx_exits=$COUNT_CHECK_EAG_EXITS;
if($dashx_exits>999999999999999){$dashx_exits=$dashx_exits / 1000000000000000 . " Quad";}
if($dashx_exits>999999999999){$dashx_exits=$dashx_exits / 1000000000000 . " Tril";}
if($dashx_exits>999999999){$dashx_exits=$dashx_exits / 1000000000 . " Bil";}
if($dashx_exits>999999){$dashx_exits=$dashx_exits / 1000000 . "M";}
if($dashx_exits>9999){$dashx_exits=$dashx_exits / 1000 . "K";}

$dashx_gothrus=$COUNT_CHECK_EAG_GOTHRUS;
if($dashx_gothrus>999999999999999){$dashx_gothrus=$dashx_gothrus / 1000000000000000 . " Quad";}
if($dashx_gothrus>999999999999){$dashx_gothrus=$dashx_gothrus / 1000000000000 . " Tril";}
if($dashx_gothrus>999999999){$dashx_gothrus=$dashx_gothrus / 1000000000 . " Bil";}
if($dashx_gothrus>999999){$dashx_gothrus=$dashx_gothrus / 1000000 . "M";}
if($dashx_gothrus>9999){$dashx_gothrus=$dashx_gothrus / 1000 . "K";}
    	...
    	<div class="dashx-inner-container">
        	<div class="dashx-inner-container-row">
            	<div class="dashx-inner-container-row-col1">
                    ...
                    <div class="dashx-inner-details-under">
                    	<div class="dashx-inner-details-under-container">
                        	<div class="dashx-inner-details-under-container-row">
                            	<div class="dashx-inner-details-under-container-row-col1">Exits</div>
                                <div class="dashx-inner-details-under-container-row-col2">GoThrus</div>
                            </div>
                            <div class="dashx-inner-details-under-container-row">
                            	<div class="dashx-inner-details-under-container-row-col1 red"><?php echo $dashx_exits;?></div>
                                <div class="dashx-inner-details-under-container-row-col2 green"><?php echo $dashx_gothrus;?></div>
                            </div>
    	                </div>
                    </div>
                    ...
                    <?php
					/* GET 5 NEW IPs */
					$CHECK_NEWIPs=mysql_query("SELECT * FROM {$properties->DB_PREFIX}tempsystem ORDER BY id DESC LIMIT 5");
