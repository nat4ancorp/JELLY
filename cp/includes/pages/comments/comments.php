...
/* --------------------------------------------------------------------------------------------------------------------------- */
case 'bulkdelete':
if(isset($_POST['undo_bulkdelete'])){
	...
	if(!empty($_POST['check_'.$pop.'[]'])) {
		foreach($_POST['check_'.$pop.'[]'] as $check) {
	...
	?>
    <form method="post">
    <br />Are you sure you want to delete all the comments you have selected (#)? 
    ...
    <?php
} /* END UNDO BULK DELETE CONDITIONAL */
break;
...
mysql_query("DELETE FROM {$properties->DB_PREFIX}".$what." WHERE id='".$commentid."' AND entry_id='".$entryid."'");
?>
<br />Okay I have permanently deleted the comment titled: <b><?php echo $title;?></b> from the Trash which was posted on the <b><?php echo ucfirst($pop);?></b> (cannot undo) <a href="<?php echo $WEBSITE_URL;?>?menu=comments" style="cursor:pointer;">Refresh</a>
<?php
} else {
...
$GET_POST=mysql_query("SELECT * FROM {$properties->DB_PREFIX}".$what." WHERE id='".$commentid."' AND entry_id='".$entryid."'");
...
?>
"<br />Okay I have left the comment titled: <b><?php echo $title;?></b> in the Trash which was posted on the <b><?php echo ucfirst($pop);?></b>. <a href="<?php echo $WEBSITE_URL;?>?menu=comments" style="cursor:pointer;">Refresh</a>
<?php
} else {
while($FETCH_POST=mysql_fetch_array($GET_POST)){
$title=$FETCH_POST['title'];
...
?>
<form method="post">
<br />Are you really sure you want to permanently delete the comment titled: <b><?php echo $title;?></b> posted on the <b><?php echo ucfirst($pop);?></b> which is currently in the trash? (you will lose all data on this post; just making sure you understand).
...
?>
<form method="post">
<br />Are you sure you want to permanently delete the comment titled: <b><?php echo $title;?></b> posted on the <b><?php echo ucfirst($pop);?> which is currently in the trash</b>? (this action cannot be undone)
<input type="radio" name="ays_answer" value="yes" onclick="this.form.submit();" /> Yes <input type="radio" name="ays_answer" value="no" onclick="this.form.submit();" /> No
</form>
<?php
}	
...
/* STEP 3: UPLOAD/SAVE */
mysql_query("UPDATE {$properties->DB_PREFIX}".$what." SET yname='".$editpost_yname."' WHERE id='".$commentid."' AND entry_id='".$entryid."'");
mysql_query("UPDATE {$properties->DB_PREFIX}".$what." SET yemail='".$editpost_yemail."' WHERE id='".$commentid."' AND entry_id='".$entryid."'");
mysql_query("UPDATE {$properties->DB_PREFIX}".$what." SET yweb='".$editpost_yweb."' WHERE id='".$commentid."' AND entry_id='".$entryid."'");
mysql_query("UPDATE {$properties->DB_PREFIX}".$what." SET mood='".$editpost_mood."' WHERE id='".$commentid."' AND entry_id='".$entryid."'");
mysql_query("UPDATE {$properties->DB_PREFIX}".$what." SET title='".$editpost_title."' WHERE id='".$commentid."' AND entry_id='".$entryid."'");
mysql_query("UPDATE {$properties->DB_PREFIX}".$what." SET content='".$editpost_content."' WHERE id='".$commentid."' AND entry_id='".$entryid."'");
mysql_query("UPDATE {$properties->DB_PREFIX}".$what." SET dateandtime='".$editpost_date." ".$editpost_time."' WHERE id='".$commentid."' AND entry_id='".$entryid."'");
mysql_query("UPDATE {$properties->DB_PREFIX}".$what." SET status='".$editpost_status."' WHERE id='".$commentid."' AND entry_id='".$entryid."'");
mysql_query("UPDATE {$properties->DB_PREFIX}".$what." SET is_searchable='".$editpost_is_searchable."' WHERE id='".$commentid."' AND entry_id='".$entryid."'");

/* STEP 4: POST RETURN OF RECEIPT */
echo "<br /><b>".$editpost_title."</b> has been successfully updated! <a href=\"".$WEBSITE_URL."?menu=comments\">Refresh</a>";
}
} else {
$commentid=$_GET['commentid'];
$entryid=$_GET['entryid'];
...
$GET_ENTRY_INFO=mysql_query("SELECT * FROM {$properties->DB_PREFIX}".$what." WHERE id='".$commentid."' AND entry_id='".$entryid."' AND status != 'Deleted'");
...
<option value="Denied" selected="selected">Denied</option>
<?php } else {?>
...
<div class="formLayoutTableRowMainAll">
<div class="formLayoutTableRowMainAllLeftCol"> Email: </div>
<div class="formLayoutTableRowMainAllRightCol">
...
$subbase=$_GET['subbase'];
$postid=$_GET['postid'];
mysql_query("UPDATE {$properties->DB_PREFIX}".$what." SET status='Pending' WHERE id='".$commentid."' AND entry_id='".$entryid."'");
echo "<br />I just un-did your last request. :) <a href=\"".$WEBSITE_URL."?menu=comments\" style=\"cursor:pointer;\">Refresh</a>";
} else {
if(isset($_POST['ays_answer'])){
/* STEP 1: GET DATA */
$commentid=$_GET['commentid'];
...
mysql_query("UPDATE {$properties->DB_PREFIX}".$what." SET status='Deleted' WHERE id='".$commentid."' AND entry_id='".$entryid."'");
?>
<form method="post">
<br />Okay I have deleted the comment titled: <b><?php echo $title;?></b> posted on the <?php echo ucfirst($pop);?> (<input type="submit" name="undo_delete" value="undo" />) <a href="<?php echo $WEBSITE_URL;?>?menu=comments" style="cursor:pointer;">Refresh</a>
</form>
<?php
} else if($_POST['ays_answer']=="no"){
echo "<br />Alright sounds fair to me, I will leave the comment titled: <b>".$title."</b> on the <b>".ucfirst($pop)."</b> alone. <a href=\"".$WEBSITE_URL."?menu=comments\" style=\"cursor:pointer;\">Refresh</a>";
}
}										
}
} else {
...
$subbase=$_GET['subbase'];
$postid=$_GET['postid'];
mysql_query("UPDATE {$properties->DB_PREFIX}".$what." SET status='Deleted' WHERE id='".$commentid."' AND entry_id='".$entryid."'") or die(mysql_error());
echo "<br />I just deleted it again, you sure are having a hard time making up your mind. :) <a href=\"".$WEBSITE_URL."?menu=comments\" style=\"cursor:pointer;\">Refresh</a>";
} else {
if(isset($_POST['ays_answer'])){
/* STEP 1: GET DATA */
$commentid=$_GET['commentid'];
...
if($_POST['ays_answer']=="yes"){
/* STEP 4: "DELETE" (really set the status to Deleted) */
mysql_query("UPDATE {$properties->DB_PREFIX}".$what." SET status='Recovered' WHERE id='".$commentid."' AND entry_id='".$entryid."'");
?>
<form method="post">
<br />Okay I have recovered the comment titled: <b><?php echo $title;?></b> from the Trash and placed it in the <?php echo ucfirst($pop);?> (<input type="submit" name="undo_recover" value="undo" />) <a href="<?php echo $WEBSITE_URL;?>?menu=comments" style="cursor:pointer;">Refresh</a>
</form>
<?php
} else if($_POST['ays_answer']=="no"){
...
$comment_id=$FETCH_POST['id'];
}
//get entry id info for email
$GET_ENTRY=mysql_query("SELECT * FROM {$properties->DB_PREFIX}".$base."_entries WHERE id='".$entryid."'");
while($FETCH_ENTRY=mysql_fetch_array($GET_ENTRY)){
$date_year=$FETCH_ENTRY['date_year'];
$date_month=$FETCH_ENTRY['date_month'];
$date_day=$FETCH_ENTRY['date_day'];
$entry_title=$FETCH_ENTRY['title'];
}
...
if($_GET['action']=="approve"){
	/* UPDATE DATABASE */
	mysql_query("UPDATE {$properties->DB_PREFIX}".$what." SET status='Approved' WHERE id='".$commentid."' AND entry_id='".$entryid."'");
	
	/* SEND EMAIL */
	$b_year=$date_year;
	$b_month=$date_month;
	$b_day=$date_day;
	$pname_title=$entry_title;	
	CENTRALIZED_EMAIL_RESPONSE_SYSTEM($properties,"comment_approved",$to,$properties->PADMAIN,$pop);
	
} else if($_GET['action']=="deny"){
	/* UPDATE DATABASE */
	mysql_query("UPDATE {$properties->DB_PREFIX}".$what." SET status='Denied' WHERE id='".$commentid."' AND entry_id='".$entryid."'");
	...
	$b_day=$date_day;
	$pname_title=$entry_title;	
	CENTRALIZED_EMAIL_RESPONSE_SYSTEM($properties,"comment_denied",$to,$properties->PADMAIN,$pop);
	
}
?>
<form method="post">
<br />Okay I have <?php if($_GET['action']=="approve"){?>approved<?php }else if($_GET['action']=="approve"){?>denied<?php } ?> the comment titled: <b><?php echo $title;?></b> posted in the <?php echo ucfirst($pop);?> (<input type="submit" name="undo_approve" value="undo" />) <a href="<?php echo $WEBSITE_URL;?>?menu=comments" style="cursor:pointer;">Refresh</a>
</form>
<?php
} else if($_POST['ays_answer']=="no"){
echo "<br />Alright sounds fair to me, I will leave the comment title: <b>".$title."</b> in the <b>".ucfirst($pop)."</b> alone. <a href=\"".$WEBSITE_URL."?menu=comments\" style=\"cursor:pointer;\">Refresh</a>";
}
}										
}
} else {
...
?>
<form method="post">
<br />Are you sure you want to approve the comment titled: <b><?php echo $title;?></b> commented on the <b><?php echo $pop;?></b>? 
<input type="radio" name="ays_answer" value="yes" onclick="this.form.submit();" /> Yes <input type="radio" name="ays_answer" value="no" onclick="this.form.submit();" /> No
</form>
<?php
}	
...
?>
<form method="post">
<br />Okay I have <?php if($_GET['action']=="approve"){?>approved<?php }else if($_GET['action']=="approve"){?>denied<?php } ?> the comment titled: <b><?php echo $title;?></b> posted in the <?php echo ucfirst($pop);?> (<input type="submit" name="undo_deny" value="undo" />) <a href="<?php echo $WEBSITE_URL;?>?menu=comments" style="cursor:pointer;">Refresh</a>
</form>
<?php
} else if($_POST['ays_answer']=="no"){
echo "<br />Alright sounds fair to me, I will leave the comment title: <b>".$title."</b> in the <b>".ucfirst($pop)."</b> alone. <a href=\"".$WEBSITE_URL."?menu=comments\" style=\"cursor:pointer;\">Refresh</a>";
...
?>
<form method="post">
<br />Are you sure you want to deny the comment titled: <b><?php echo $title;?></b> commented on the <b><?php echo $pop;?></b>? 
<input type="radio" name="ays_answer" value="yes" onclick="this.form.submit();" /> Yes <input type="radio" name="ays_answer" value="no" onclick="this.form.submit();" /> No
</form>
<?php
}	
...
<div class="formLayoutFullRow InBetween">
<div class="formLayoutFullCol InBetween valignMiddle"> No Comments were found...it means no one loves you. Ha ha jk :(</div>
</div>
</div>
<?php	
} else {
...
<div class="formLayoutFullCol alignLeft width-midsmall valignMiddle BorderLeft BorderBottom BorderTop">
Master Menu
</div>
<div class="formLayoutFullCol aligncenter width-midsmall valignMiddle BorderLeft BorderBottom BorderTop BorderRight">
<a href="?menu=comments&page=comments&action=bulkdelete&what=<?php echo $item."_".$sub_item;?>&pad=<?php echo $pad;?>&pop=<?php echo $page;?>&base=<?php echo $item;?>&subbase=<?php if($sub_item==""){$sub_item="null";}echo $sub_item;?>">Bulk Delete</a>
</div>
</div>
</div>
...
<div class="formLayoutFullCol alignLeft width-medium valignMiddle fontBig BorderTop BorderBottom"> <a href="?menu=comments&page=comments&order=title#<?php echo $ITEMS_LIST_LIST[$i];?>">Title</a>
<?php if(isset($_GET['direction']) && $_GET['direction']!=""){if($_GET['direction']=="ASC"){?>
[<a href="?menu=comments&page=comments&order=<?php if(isset($_GET['order']) && $_GET['order']!=""){echo $_GET['order'];}else{echo $DEFAULT_ORDER;}?>&direction=DESC#<?php echo $ITEMS_LIST_LIST[$i];?>">ASC</a>]
<?php }else if($_GET['direction']=="DESC"){?>
[<a href="?menu=comments&page=comments&order=<?php if(isset($_GET['order']) && $_GET['order']!=""){echo $_GET['order'];}else{echo $DEFAULT_ORDER;}?>&direction=ASC#<?php echo $ITEMS_LIST_LIST[$i];?>">DESC</a>]
<?php }}else{?>
[<a href="?menu=comments&page=comments&order=<?php if(isset($_GET['order']) && $_GET['order']!=""){echo $_GET['order'];}else{if($DEFAULT_ORDER=="dateandtime OR dateandtime_goingtostart"){echo "dateandtime";}else{echo $DEFAULT_ORDER;}}?>&direction=ASC#<?php echo $ITEMS_LIST_LIST[$i];?>">DESC</a>]
<?php }?>
</div>

<div class="formLayoutFullCol alignLeft width-semimedium valignMiddle fontBig BorderTop BorderBottom"> <a href="?menu=comments&page=comments&order=commenter">Poster</a> 
<?php if(isset($_GET['direction']) && $_GET['direction']!=""){if($_GET['direction']=="ASC"){?>
[<a href="?menu=comments&page=comments&order=<?php if(isset($_GET['order']) && $_GET['order']!=""){echo $_GET['order'];}else{echo $DEFAULT_ORDER;}?>&direction=DESC#<?php echo $ITEMS_LIST_LIST[$i];?>">ASC</a>]
<?php }else if($_GET['direction']=="DESC"){?>
[<a href="?menu=comments&page=comments&order=<?php if(isset($_GET['order']) && $_GET['order']!=""){echo $_GET['order'];}else{echo $DEFAULT_ORDER;}?>&direction=ASC#<?php echo $ITEMS_LIST_LIST[$i];?>">DESC</a>]
...
<div class="formLayoutFullCol alignLeft width-semimedium valignMiddle fontBig BorderTop BorderBottom"> <a href="?menu=comments&page=comments&order=mood">Mood</a> 
<?php if(isset($_GET['direction']) && $_GET['direction']!=""){if($_GET['direction']=="ASC"){?>
[<a href="?menu=comments&page=comments&order=<?php if(isset($_GET['order']) && $_GET['order']!=""){echo $_GET['order'];}else{echo $DEFAULT_ORDER;}?>&direction=DESC#<?php echo $ITEMS_LIST_LIST[$i];?>">ASC</a>]
<?php }else if($_GET['direction']=="DESC"){?>
[<a href="?menu=comments&page=comments&order=<?php if(isset($_GET['order']) && $_GET['order']!=""){echo $_GET['order'];}else{echo $DEFAULT_ORDER;}?>&direction=ASC#<?php echo $ITEMS_LIST_LIST[$i];?>">DESC</a>]
...
<div class="formLayoutFullCol alignLeft width-xsmall valignMiddle fontBig BorderTop BorderBottom BorderRight"> <a href="?menu=comments&page=comments&order=dateandtime">Date</a> 
<?php if(isset($_GET['direction']) && $_GET['direction']!=""){if($_GET['direction']=="ASC"){?>
[<a href="?menu=comments&page=comments&order=<?php if(isset($_GET['order']) && $_GET['order']!=""){echo $_GET['order'];}else{echo $DEFAULT_ORDER;}?>&direction=DESC#<?php echo $ITEMS_LIST_LIST[$i];?>">ASC</a>]
<?php }else if($_GET['direction']=="DESC"){?>
[<a href="?menu=comments&page=comments&order=<?php if(isset($_GET['order']) && $_GET['order']!=""){echo $_GET['order'];}else{echo $DEFAULT_ORDER;}?>&direction=ASC#<?php echo $ITEMS_LIST_LIST[$i];?>">DESC</a>]
...
<input type="checkbox" name="check_<?php echo $ITEMS_LIST_LIST[$i];?>[]" />
</div>
...
<?php if($status=="Deleted"){
	?>
    <a href="?menu=comments&page=comments&action=recover&what=<?php echo $item."_".$sub_item;?>&pad=<?php echo $pad;?>&pop=<?php echo $page;?>&commentid=<?php echo $id;?>&entryid=<?php echo $entry_id;?>&base=<?php echo $item;?>&subbase=<?php if($sub_item==""){$sub_item="null";}echo $sub_item;?>">Recover</a>
     | 
    <a href="?menu=comments&page=comments&action=deleteperm&what=<?php echo $item."_".$sub_item;?>&pad=<?php echo $pad;?>&pop=<?php echo $page;?>&commentid=<?php echo $id;?>&entryid=<?php echo $entry_id;?>&base=<?php echo $item;?>&subbase=<?php if($sub_item==""){$sub_item="null";}echo $sub_item;?>">Delete Permanently</a>
	<?php
    } else {
	?>
    <a href="?menu=comments&page=comments&action=edit&what=<?php echo $item."_".$sub_item;?>&pad=<?php echo $pad;?>&pop=<?php echo $page;?>&commentid=<?php echo $id;?>&entryid=<?php echo $entry_id;?>&base=<?php echo $item;?>&subbase=<?php if($sub_item==""){$sub_item="null";}echo $sub_item;?>">Edit</a>
     | 
    <a href="?menu=comments&page=comments&action=delete&what=<?php echo $item."_".$sub_item;?>&pad=<?php echo $pad;?>&pop=<?php echo $page;?>&commentid=<?php echo $id;?>&entryid=<?php echo $entry_id;?>&base=<?php echo $item;?>&subbase=<?php if($sub_item==""){$sub_item="null";}echo $sub_item;?>">Delete</a>
	<?php
    }
	?>
    <?php
	if(getGlobalVars($properties,"post_action_for_comments")=="Subject to Approval"){
		?>
		|
		<?php if($status=="Approved" || $status=="Deleted" || $status=="Recovered"){
			?>
			<del>Approve</del>
			<?php
		} else {
			?>
			<a href="?menu=comments&page=comments&action=approve&what=<?php echo $item."_".$sub_item;?>&pad=<?php echo $pad;?>&pop=<?php echo $page;?>&commentid=<?php echo $id;?>&entryid=<?php echo $entry_id;?>&base=<?php echo $item;?>&subbase=<?php if($sub_item==""){$sub_item="null";}echo $sub_item;?>">Approve</a>
			<?php
		}
		?>
		|
		<?php if($status=="Denied" || $status=="Deleted" || $status=="Recovered"){
			?>
			<del>Deny</del>
			<?php
		} else {
			?>
			<a href="?menu=comments&page=comments&action=deny&what=<?php echo $item."_".$sub_item;?>&pad=<?php echo $pad;?>&pop=<?php echo $page;?>&commentid=<?php echo $id;?>&entryid=<?php echo $entry_id;?>&base=<?php echo $item;?>&subbase=<?php if($sub_item==""){$sub_item="null";}echo $sub_item;?>">Deny</a>
			<?php
		}
		?>
		<?php
	} else {
		/* COMMENTS ARE EITHER IMMEDIATELY POSTED OR POSTED AFTER 24 HOURS */
	}
	?>     
    
    </div>
<div class="formLayoutFullCol alignLeft width-semimedium valignTop BorderBottom BorderTop"> <?php if($yweb!=$WEBSITE_URL){echo "<a href=\"".$yweb."\">".$yname."</a>";}else{echo $yname;}?> </div>
