...
if(isset($_POST['save'])){
	...
	$data_fb_like=mysql_real_escape_string($_POST['data_fb_like']);
	
	$data_avatar=mysql_real_escape_string($_POST['data_avatar']);
	...
	if($error_console != "") {
		...
		if($data_oldpass == "" || $data_newpass == "" || $data_confirmpass == ""){/* DO NOT UPDATE */}else{mysql_query("UPDATE {$properties->DB_PREFIX}users SET upass = '$data_newupass' WHERE id='".$user_id."'");}
		...
		mysql_query("UPDATE {$properties->DB_PREFIX}users SET fb_like = '$data_fb_like' WHERE id='".$user_id."'");
		mysql_query("UPDATE {$properties->DB_PREFIX}users SET avatar = '$data_avatar' WHERE id='".$user_id."'");
	...
	if(mysql_num_rows($GET_DATA)<1){
		...
		while($FETCH_USER_DATA=mysql_fetch_array($GET_DATA)){
			...
			$data_fb_like=$FETCH_USER_DATA['fb_like'];
			$data_level=$FETCH_USER_DATA['level'];
			$data_avatar=$FETCH_USER_DATA['avatar'];
		}
		?>
            ...
            <fieldset>
                ...
                <div class="formLayoutTableMainAll">
                    ...
                    <div class="formLayoutTableRowMainAll">
                        ...
                        <div class="formLayoutTableRowMainAllRightCol">
                            <select name="data_gender" onchange="avatar('changegender','','<?php echo $WEBSITE_URL_ROOT;?>','<?php echo $THEME_NAME;?>',this.value)">
                    ...
                    <div class="formLayoutTableRowMainAll">
                        ...
                        <div class="formLayoutTableRowMainAllRightCol">
                        	<div style="border: thin dashed #666; width: 378px; margin-bottom: 10px; padding: 10px 10px 5px 10px;">
                            <span id="gravatar" style="position: relative; top: -20px;">Current:</span>
                            <?php
							if($data_avatar=="" || $data_avatar=="male" || $data_avatar=="female" || $data_avatar=="other"){
								/* NO AVATAR */
								switch($data_gender){
									case 'male':
										?><img id="avatar-current" src="<?php echo $WEBSITE_URL_ROOT.$THEME_NAME;?>images/profile_male.png" width="50" height="50" alt="Your Avatar" class="selected" /><?php
									break;
									case 'female':
										?><img id="avatar-current" src="<?php echo $WEBSITE_URL_ROOT.$THEME_NAME;?>images/profile_female.png" width="50" height="50" alt="Your Avatar" class="selected" /><?php
									break;
									case 'other':
										?><img id="avatar-current" src="<?php echo $WEBSITE_URL_ROOT.$THEME_NAME;?>images/profile_other.png" width="50" height="50" alt="Your Avatar" class="selected" /><?php
									break;
								}
							} else {
								/* FIND THE AVATAR IN THE AVATARS FOLDER */
								?><img id="avatar-current" src="<?php echo $WEBSITE_URL_ROOT;?>includes/public/avatars/<?php echo $data_avatar;?>.png" width="50" height="50" alt="Your Avatar" class="selected" /><?php
							}
							
							$GET_AVATARS=mysql_query("SELECT * FROM {$properties->DB_PREFIX}avatars ORDER BY name");
							?>
                            <script type="text/javascript">
							function avatar(action,id,weburl,themename,imagename){
								switch(action){
									case 'changegender':
										var male 	= '<?php echo $WEBSITE_URL_ROOT.$THEME_NAME;?>images/profile_male.png';
										var female	= '<?php echo $WEBSITE_URL_ROOT.$THEME_NAME;?>images/profile_female.png';
										var other	= '<?php echo $WEBSITE_URL_ROOT.$THEME_NAME;?>images/profile_other.png'
										
										if((document.getElementById("avatar-current").src==male) || (document.getElementById("avatar-current").src==female) || (document.getElementById("avatar-current").src==other)){											
											document.getElementById("avatar-0").setAttribute('src',weburl+themename+'images/profile_'+imagename+'.png');	
											document.getElementById("avatar-current").setAttribute('src',weburl+themename+'images/profile_'+imagename+'.png');											
										} else {											
											/* SELECTED AN AVATAR; DON'T CHANGE CURRENT */
											document.getElementById("avatar-0").setAttribute('src',weburl+themename+'images/profile_'+imagename+'.png');	
										}
										
										document.getElementById("avatar-0").setAttribute('onclick','avatar(\'click\',this.id,\'<?php echo $WEBSITE_URL_ROOT;?>\',\'<?php echo $THEME_NAME;?>\',\''+imagename+'\')');																				
										
										document.getElementById("set_avatar").value=imagename;
									break;
									case 'mouseover':
										if(document.getElementById(id).className == "selected"){/* SELECTED; DO NOT HOVER OUT */}else{document.getElementById(id).setAttribute('class','hover');}
									break;
									case 'mouseout':
										if(document.getElementById(id).className == "selected"){/* SELECTED; DO NOT HOVER OUT */}else{document.getElementById(id).setAttribute('class','notselected');}
									break;
									case 'click':
										if(imagename=="male" || imagename=="female" || imagename=="other"){
											/* USER SELECTED NO AVATAR */
											document.getElementById("avatar-current").setAttribute('src',weburl+themename+'images/profile_'+imagename+'.png');
										} else {
											document.getElementById("avatar-current").setAttribute('src',weburl+'includes/public/avatars/'+imagename+'.png');	
										}
										document.getElementById(id).setAttribute('class','selected');
										document.getElementById("set_avatar").value=imagename;
										
										//clear out all other selected ones
										for(i = 0; i <= <?php echo mysql_num_rows($GET_AVATARS);?>; i++){
											if(document.getElementById("avatar-"+i).getAttribute('id')==id){
												/* ITEM SELECTED */
											} else {
												document.getElementById("avatar-"+i).setAttribute('class','notselected');	
											}
										}
									break;
								}
                            ...
                            <br />                           
                            <input type="hidden" name="data_avatar" id="set_avatar" value="<?php echo $data_avatar;?>" />
                            <span style="font-size:1.25em;">Choose an Avatar</span>
                            <div class="avatar-table">
                            	<div class="avatar-table-row">
                                	<?php
									switch($data_gender){
										case 'male':
											?><img id="avatar-0" class="<?php if($data_avatar=="" && $data_gender=="male"){?>selected<?php }else{?>notselected<?php }?> cursor" src="<?php echo $WEBSITE_URL_ROOT.$THEME_NAME;?>images/profile_male.png" width="50" height="50" alt="Male Avatar" title="No Avatar" onclick="avatar('click',this.id,'<?php echo $WEBSITE_URL_ROOT;?>','<?php echo $THEME_NAME;?>','male')" onmouseover="avatar('mouseover',this.id,'<?php echo $WEBSITE_URL_ROOT;?>','<?php echo $THEME_NAME;?>','')" onmouseout="avatar('mouseout',this.id,'<?php echo $WEBSITE_URL_ROOT;?>','<?php echo $THEME_NAME;?>','')" /><?php
										break;
										case 'female':
											?><img id="avatar-0" class="<?php if($data_avatar=="" && $data_gender=="female"){?>selected<?php }else{?>notselected<?php }?> cursor" src="<?php echo $WEBSITE_URL_ROOT.$THEME_NAME;?>images/profile_female.png" width="50" height="50" alt="Female Avatar" title="No Avatar" onclick="avatar('click',this.id,'<?php echo $WEBSITE_URL_ROOT;?>','<?php echo $THEME_NAME;?>','female')" onmouseover="avatar('mouseover',this.id,'<?php echo $WEBSITE_URL_ROOT;?>','<?php echo $THEME_NAME;?>','')" onmouseout="avatar('mouseout',this.id,'<?php echo $WEBSITE_URL_ROOT;?>','<?php echo $THEME_NAME;?>','')" /><?php
										break;
										case 'other':
											?><img id="avatar-0" class="<?php if($data_avatar=="" && $data_gender=="other"){?>selected<?php }else{?>notselected<?php }?> cursor" src="<?php echo $WEBSITE_URL_ROOT.$THEME_NAME;?>images/profile_other.png" width="50" height="50" alt="Other Avatar" title="No Avatar" onclick="avatar('click',this.id,'<?php echo $WEBSITE_URL_ROOT;?>','<?php echo $THEME_NAME;?>','other')" onmouseover="avatar('mouseover',this.id,'<?php echo $WEBSITE_URL_ROOT;?>','<?php echo $THEME_NAME;?>')" onmouseout="avatar('mouseout',this.id,'<?php echo $WEBSITE_URL_ROOT;?>','<?php echo $THEME_NAME;?>','')" /><?php
										break;
									}
									
									/* GET ALL AVATARS FROM DB */
									$GET_AVATARS=mysql_query("SELECT * FROM {$properties->DB_PREFIX}avatars ORDER BY name");
									if(mysql_num_rows($GET_AVATARS)<1){
										/* NO AVATARS */	
									} else {
										$counter=0;
										while($FETCH_AVATARS=mysql_fetch_array($GET_AVATARS)){
											/* CHECK FOR AVATAR AVAIL */
											if($FETCH_AVATARS['avail']=="public"){
												/* A PUBLIC AVATAR */
												if(++$counter == mysql_num_rows($GET_AVATARS)){
													?>
													<img id="avatar-<?php echo $FETCH_AVATARS['id'];?>" class="notselected cursor" src="<?php echo $WEBSITE_URL_ROOT;?>includes/public/avatars/<?php echo strtolower($FETCH_AVATARS['name']);?>.png" width="50" height="50" alt="" onclick="avatar('click',this.id,'<?php echo $WEBSITE_URL_ROOT;?>','<?php echo $THEME_NAME;?>','<?php echo strtolower($FETCH_AVATARS['name']);?>')" onmouseover="avatar('mouseover',this.id,'<?php echo $WEBSITE_URL_ROOT;?>','<?php echo $THEME_NAME;?>','')" onmouseout="avatar('mouseout',this.id,'<?php echo $WEBSITE_URL_ROOT;?>','<?php echo $THEME_NAME;?>','')" title="<?php echo $FETCH_AVATARS['name'];?>" />
													<?php
												} else {
													?>
													<img id="avatar-<?php echo $FETCH_AVATARS['id'];?>" class="notselected cursor" src="<?php echo $WEBSITE_URL_ROOT;?>includes/public/avatars/<?php echo strtolower($FETCH_AVATARS['name']);?>.png" width="50" height="50" alt="" onclick="avatar('click',this.id,'<?php echo $WEBSITE_URL_ROOT;?>','<?php echo $THEME_NAME;?>','<?php echo strtolower($FETCH_AVATARS['name']);?>')" onmouseover="avatar('mouseover',this.id,'<?php echo $WEBSITE_URL_ROOT;?>','<?php echo $THEME_NAME;?>','')" onmouseout="avatar('mouseout',this.id,'<?php echo $WEBSITE_URL_ROOT;?>','<?php echo $THEME_NAME;?>','')" title="<?php echo $FETCH_AVATARS['name'];?>" />
											...
											} else if($FETCH_AVATARS['avail']=="private") {
												/* A PRIVATE AVATAR; CHECK TO SEE IF AVATAR BELONGS TO USER */
												if($user_id==$FETCH_AVATARS['owner']){
													/* USER OWNS AVATAR; DISPLAY */
													if(++$counter == mysql_num_rows($GET_AVATARS)){
														?>
														<img id="avatar-<?php echo $FETCH_AVATARS['id'];?>" class="notselected cursor" src="<?php echo $WEBSITE_URL_ROOT;?>includes/public/avatars/<?php echo strtolower($FETCH_AVATARS['name']);?>.png" width="50" height="50" alt="" onclick="avatar('click',this.id,'<?php echo $WEBSITE_URL_ROOT;?>','<?php echo $THEME_NAME;?>','<?php echo strtolower($FETCH_AVATARS['name']);?>')" onmouseover="avatar('mouseover',this.id,'<?php echo $WEBSITE_URL_ROOT;?>','<?php echo $THEME_NAME;?>','')" onmouseout="avatar('mouseout',this.id,'<?php echo $WEBSITE_URL_ROOT;?>','<?php echo $THEME_NAME;?>','')" title="<?php echo $FETCH_AVATARS['name'];?>" />
														<?php
														...
														?>
														<img id="avatar-<?php echo $FETCH_AVATARS['id'];?>" class="notselected cursor" src="<?php echo $WEBSITE_URL_ROOT;?>includes/public/avatars/<?php echo strtolower($FETCH_AVATARS['name']);?>.png" width="50" height="50" alt="" onclick="avatar('click',this.id,'<?php echo $WEBSITE_URL_ROOT;?>','<?php echo $THEME_NAME;?>','<?php echo strtolower($FETCH_AVATARS['name']);?>')" onmouseover="avatar('mouseover',this.id,'<?php echo $WEBSITE_URL_ROOT;?>','<?php echo $THEME_NAME;?>','')" onmouseout="avatar('mouseout',this.id,'<?php echo $WEBSITE_URL_ROOT;?>','<?php echo $THEME_NAME;?>','')" title="<?php echo $FETCH_AVATARS['name'];?>" />&nbsp
														<?php	
													}
												} else {
													/* USER DOES NOT OWN AVATAR; DONT DISPLAY */
												}
											}
										}
									}
                            ...
                            <span style="font-size:1.25em;">or Upload One</span>
                            <script type="text/javascript">
								$('#upload').ajaxupload({
								...
								<div id="upload"></div>
								<center>
								<div id="drop_here" style="width:100%;height:50px;border:1px dashed black; border-radius: 5px;">or drop files here</div>
								</center>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="formLayoutTableRowMainAll">
                        ...
                        <div class="formLayoutTableRowMainAllRightCol">
                            ...
                            if($data_issearchable=="yes"){
                                ?>
                                <input type="radio" name="data_issearchable" value="yes" checked="checked" class="radio" /> Yes <input type="radio" name="data_issearchable" value="no" class="radio" /> No
                                <?php
                            } else {
                                ?>
                                <input type="radio" name="data_issearchable" value="yes" class="radio" /> Yes <input type="radio" name="data_issearchable" value="no" checked="checked" class="radio" /> No
                                <?php
                            }
                            ?>
                        </div>
                    ...
                    <div class="formLayoutTableRowMainAll">
                        ...
                        <div class="formLayoutTableRowMainAllRightCol">
                            ...
                            if($data_fb_like=="yes"){
                                ?>
                                <input type="radio" name="data_fb_like" value="yes" checked="checked" class="radio" <?php if(($type=="admin" && $head_admin=="yes") || ($data_iswebmaster=="yes" || $data_iswebmasterbydefault=="yes")){?><?php }else{?>disabled="disabled"<?php }?> /> Yes <input type="radio" name="data_fb_like" value="no" class="radio" <?php if(($type=="admin" && $head_admin=="yes") || ($data_iswebmaster=="yes" || $data_iswebmasterbydefault=="yes")){?><?php }else{?>disabled="disabled"<?php }?> /> No
                                <?php
                            } else {
                                ?>
                                <input type="radio" name="data_fb_like" value="yes" class="radio" <?php if(($type=="admin" && $head_admin=="yes") || ($data_iswebmaster=="yes" || $data_iswebmasterbydefault=="yes")){?><?php }else{?>disabled="disabled"<?php }?> /> Yes <input type="radio" name="data_fb_like" value="no" checked="checked" class="radio" <?php if(($type=="admin" && $head_admin=="yes") || ($data_iswebmaster=="yes" || $data_iswebmasterbydefault=="yes")){?><?php }else{?>disabled="disabled"<?php }?> /> No
                                <?php
                            }
                            ?> * This will "trick" the system into thinking you haven't liked the Facebook Page
                        </div>
                    </div>
